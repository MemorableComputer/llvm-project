...
;
; An unknown tune-cpu is only a WARNING ("not a recognized processor")
; that silently falls back to generic scheduling, so the first RUN pins
; the tune CPU's existence by requiring an empty stderr-side warning
; stream; the second checks codegen on stdout.
;
...
;
; WARN-NOT: not a recognized processor
...
define <vscale x 2 x float> @interleave(<vscale x 2 x float> %a, <vscale x 2 x float> %b, i64 %n, ptr %p) #0 {
entry:
  br label %loop
